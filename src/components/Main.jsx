...
export const Main = () => {
    ...
    const { data: pokemons, isLoading, isError, error, isFetched } = useQuery({
        queryFn: async () => {
    ...
    return (
        <div>
            {isLoading ? (<Preloader />) : (
                <motion.div className="bg-[#FF6F40] grid grid-rows-5 lg:grid-cols-7 xl:max-w-6xl h-[800px] border-4 border-black  rounded-2xl overflow-hidden mx-auto mt-3 p-4"
                    initial={{ width: 0 }}
                    animate={{ width: "auto" }}
                    ...
                    <div className="row-span-3 lg:col-span-5 lg:row-span-5 pt-2 md:p-2 lg:order-2 relative"><div className="visible lg:hidden"><img src={pokelogLogo} alt="" className="m-auto md:m-0 w-[120px] md:w-[160px]" /></div>
                        {/* Pokeball logo */}
                        <motion.div className="absolute top-[50%] left-[50%] translate-y-[-50%] translate-x-[-50%] z-0"
                            initial={{ opacity: 0 }}
                            animate={{ opacity: 1 }}
                        ...
                        <div className="m-auto absolute top-[50%] left-[50%] translate-y-[-50%] translate-x-[-50%] w-full h-full z-1">
                            {selectedPokemon ? <PreviewPokemon selectedPokemon={selectedPokemon} /> : (
                                <div className="h-full flex items-center justify-center">
                                    <motion.p className="text-[18px] md:text-[25px] font-bold text-white"
                                    >Select a Pokemon</motion.p>
                                </div>
                            )}
                        </div>
                    </div>
