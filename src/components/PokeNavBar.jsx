    ...
    return (
        <>
            <div className="row-span-2 lg:col-span-2 lg:row-span-5 bg-[#FF6F40] border-4 border-black rounded-xl lg:order-1 overflow-hidden">
                ...
                <div className="hidden lg:block"><input type="text" className="bg-[#2E2E2E] w-full px-4 py-2 border-black focus:outline-none text-white placeholder-slate-300" placeholder="Search pokemon..." onChange={(e) => setSearchInput(e.target.value)} /></div>
                <ul className="grid lg:block grid-cols-3 lg:grid-cols-none gap-[4px] p-2 overflow-scroll scrollbar-hide bg-[#FF6F40] h-[350px] lg:h-[610px]" style={{ gridTemplateRows: "auto" }}>
                    {props.pokemons.filter((pokemon, index) => {
                        return searchInput.toLowerCase() === '' ? pokemon : pokemon.name.toLowerCase().includes(searchInput)
                    }).map((pokemon, index) => {
                        return (
                            <li className="bg-black h-[140px] md:h-[160px] lg:h-[85px] rounded-lg relative cursor-pointer" key={pokemon.id} onClick={() => props.clickedPokemon(pokemon.id)}>
